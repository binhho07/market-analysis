...
import { osmGeocode, osmNearbyPlaces, osmLookupPlace } from "@/lib/osm";
...
export async function geocodeAddress(address: string): Promise<GeocodingResult | null> {
  if (GOOGLE_MAPS_API_KEY) {
    try {
      ...
      console.warn("Google geocoding unavailable, using OpenStreetMap:", googleMessage || error);
    }
  }
...
export async function searchNearbyPlaces(
  ...
  if (GOOGLE_MAPS_API_KEY) {
    try {
      ...
      console.warn("Google Places unavailable, using OpenStreetMap:", error);
    }
  }

  return osmNearbyPlaces(location.lat, location.lng, radiusMeters);
}
...
export async function getPlaceDetails(placeId: string): Promise<PlaceSearchResult | null> {
  ...
  try {
    ...
    console.warn("Google place details unavailable:", error);
    return osmLookupPlace(placeId);
  }
}
